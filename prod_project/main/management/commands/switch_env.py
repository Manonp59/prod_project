# your_project/your_app/management/commands/switch_env.py
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument('new_value', type=str,
                            help='New value for the environment variable')
        parser.add_argument(
            '--env', type=str, help='Path to the environment file', default='.env')

    def handle(self, *args, **options):
        variable_name = options['variable_name']
        new_value = options['new_value']
        env_file_path = options['env']

        # Obtenez le chemin absolu du fichier .env
        env_file_path = os.path.join(os.path.dirname(os.path.dirname(
            os.path.dirname(os.path.abspath(__file__)))), '..', '.env')

        set_key(env_file_path, variable_name, new_value)

        self.stdout.write(self.style.SUCCESS(
            f'Successfully changed {variable_name} to {new_value}'))
